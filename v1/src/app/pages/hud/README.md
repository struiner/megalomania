...
- HUD minimap sizing preserves a 512px baseline via a 64×64 tile preview sourced from `TilemapService` and scaled deterministically; scaling clamps to 0.25–0.5 with snapping to preferred steps (0.25/0.3125/0.375/0.5) based on the desired 160px target size. Letterboxing now cushions smaller render sizes inside a fixed frame; TODO remains for textured vs. flat fill treatment and constrained-device policy.
