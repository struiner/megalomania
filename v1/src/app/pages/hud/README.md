...
- Route guarding for `/game/interface/:panel` now evaluates feature-flag/init predicates through `HudAvailabilityService` (still stubbed) with in-HUD messaging for blocked panels.
- The overlay shell is draggable (snap-to-4px grid) to keep dialogs from occluding the world viewport; bottom HUD uses fixed anchoring with padding reserves baked into the page layout.
